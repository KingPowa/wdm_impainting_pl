...
class WDM(LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.global_batch = self.global_step
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('time/total', t_total, on_step=True, on_epoch=True)
        self.log('loss/MSE', lossmse.item(), on_step=True, on_epoch=True)
    ...
    def _step(self, image, cond, label=None, label_cond=None, label_cond_dilated=None):
        ...
        if not torch.isfinite(lossmse): #infinite
            if not torch.isfinite(torch.tensor(param_max_norm)):
                logger.error(f"Model parameters contain non-finite value {param_max_norm}, entering breakpoint", level=logger.ERROR)
                breakpoint()
            else:
                logger.warn(f"Model parameters are finite, but loss is not: {lossmse}"
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if "mse_wav" in losses:
                ...
                if self.mode=="Conditional_always_known_only_healthy_stats_roi":
                    self.log('loss/stats_loss', losses["stats_loss"][0].item(), on_step=True, on_epoch=True)
                    self.log('loss/mse_label_cond', losses["mse_label_cond"][0].item(), on_step=True, on_epoch=True)
    ...
    def log_loss_dict(self, ts, losses):
        for key, values in losses.items():
            logger.logkv_mean(f"{key}_mean", values.mean().item())
            # Log the quantiles (four quartiles, in particular).
            for sub_t, sub_loss in zip(ts.cpu().numpy(), values.detach().cpu().numpy()):
                quartile = int(4 * sub_t / self.diffusion.num_timesteps)
                logger.logkv_mean(f"{key}_q{quartile}_mean", sub_loss)
